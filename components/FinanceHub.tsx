...
// Mock data for demo - can be replaced with Firestore later
const MOCK_CASH_FLOW: CashFlowPoint[] = [
  { month: 'Ago', inflows: 42000, outflows: 28000 },
  { month: 'Set', inflows: 38000, outflows: 32000 },
  { month: 'Out', inflows: 45000, outflows: 29000 },
  { month: 'Nov', inflows: 52000, outflows: 35000 },
  { month: 'Dez', inflows: 48000, outflows: 31000 },
  { month: 'Jan', inflows: 55000, outflows: 33000 },
];
...
const MOCK_TRANSACTIONS: FinanceTransaction[] = [
  ...
  {
    id: '5',
    description: 'Projeto Lilian Designer',
    category: 'Receitas de Projetos',
    dueDate: '2026-01-30',
    amount: 17800,
    currency: 'BRL',
    type: 'income',
...
const FinanceHub: React.FC = () => {
  const [transactions, setTransactions] = useState<FinanceTransaction[]>(MOCK_TRANSACTIONS);
  ...
  const { totalBalance, receivables30, payablesTodayTomorrow } = useMemo(() => {
    const now = new Date();
    ...
    transactions.forEach((t) => {
      ...
      if (t.type === 'income') {
        balance += t.status === 'received' ? t.amount : 0;
        if (t.status !== 'received' && due <= in30Days && due >= now) receivables += t.amount;
      } else {
        balance -= t.status === 'paid' ? t.amount : 0;
        if (t.status !== 'paid' && (t.dueDate === today || t.dueDate === tomorrow)) payables += t.amount;
      }
    });
  ...
  const maxFlow = Math.max(
    ...MOCK_CASH_FLOW.flatMap((p) => [p.inflows, p.outflows]),
    1
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-5 rounded-2xl bg-neutral-900/60 border border-neutral-800 backdrop-blur-xl shadow-inner ring-1 ring-white/5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold uppercase text-neutral-400 tracking-wider">Saldo Total</span>
            </div>
            <p className="text-2xl font-mono font-black text-white">{formatCurrency(totalBalance)}</p>
          </div>
          <div className="p-5 rounded-2xl bg-neutral-900/60 border border-neutral-800 backdrop-blur-xl shadow-inner ring-1 ring-white/5">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp size={18} className="text-lime-500" />
              <span className="text-xs font-bold uppercase text-neutral-400 tracking-wider">A Receber (30 dias)</span>
            </div>
            <p className="text-2xl font-mono font-black text-lime-400">{formatCurrency(receivables30)}</p>
          </div>
          <div className="p-5 rounded-2xl bg-neutral-900/60 border border-neutral-800 backdrop-blur-xl shadow-inner ring-1 ring-white/5">
            <div className="flex items-center gap-2 mb-2">
                ...
                {filteredTransactions.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span
                          className={`inline-block px-2 py-1 rounded text-xs font-bold border ${CATEGORY_COLORS[t.category]}`}
                        >
                          {t.category}
                        </span>
                      </td>
